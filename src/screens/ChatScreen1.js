...
const ChatScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    // console.log(route.params.item.userId, 'userid')
    setUserChatMessagesLoading(true);
  ...
  const getAllMessages = async () => {
    const docid = "user" + userData?.id;

    const querySanp = await firestore()
      .collection("chatrooms")
      .doc(docid)
      .collection("sender")
      .doc(`sender_${authState?.loginedUser?.id}`)
      .collection("messages")
      .orderBy("createdAt", "desc")
      .get();
    console.log("data++++++++++++++++", querySanp);
  ...
  useEffect(() => {
    ...
    if (userData) {
      ...
      const docidOwn = "user_" + authState?.loginedUser?.id;

      console.log(docid, "initial render");
      const messageRef = firestore()
        ...
        .collection("messages")
        .orderBy("createdAt", "desc");
      console.log("mesaage data+++++++++++++", messageRef);

      const messageRefOwn = firestore()
        .collection("Chats")
        .doc(docidOwn)
        .collection("sender")
        .doc(`sender_${userData?.id}`)
        .collection("messages")
        .orderBy("createdAt", "desc");
      ...
      const unSubscribe1 = messageRefOwn.onSnapshot((querySnap) => {
        const allmsg = querySnap.docs.map((docSanp) => {
          const data = docSanp.data();
        ...
        console.log(allmsg, "allmessages new sdfjf lksfls jfakldf ");

        // setMessages([...messages, allmsg])
      });
